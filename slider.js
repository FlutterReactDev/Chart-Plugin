...
export default function slider(root, data, changeMainCanvas) {
  ...
  $leftResizer.addEventListener("mousedown", function (e) {
    ...
    document.onmousemove = function (e) {
      ...
      const left =
        WIDTH - (lastElementPoints.width + delta) - lastElementPoints.right;
  ...
  $rightResizer.addEventListener("mousedown", function (e) {
    ...
    document.onmousemove = function (e) {
      ...
      const right =
        WIDTH - lastElementPoints.width + delta - lastElementPoints.left;
  ...
  function paint(availableLines) {

    const [yMin, yMax] = computeBoundaries(data);
    const yRatio = VIEW_HEIGHT / yMax;
    const xRatio = VIEW_WIDTH / (data.columns[0].length - 2);
    createLines(ctx, data, xRatio, yRatio, DPI_HEIGHT, availableLines);
  }
...
function createLines(ctx, data, xRatio, yRatio, DPI_HEIGHT, availableLines) {
  const lines = getLineData(data, xRatio, yRatio, DPI_HEIGHT, availableLines);
  console.log(lines);
...
function getLineData(
  ...
  function createCoords(data, xRatio, yRatio) {
    return data.map((y, i) => [
      Math.floor(i * xRatio),
      Math.floor(DPI_HEIGHT - y * yRatio),
    ]);
  }
